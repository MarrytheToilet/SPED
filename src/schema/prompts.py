...
# ---------------------------------------------------------------------------
# 旧候选式 proposer（兼容保留）：阅读单篇论文，提出候选字段
# ---------------------------------------------------------------------------
...
- 研究对象：样品/材料/对象/系统/人群/数据集/场景等，按领域简介判断什么是“对象”。
- 对象属性：组成、结构、几何、分组、来源、制备/构建/采样方式、关键前处理。
- 方法与过程：实验、测量、计算、标注、训练、仿真、调查或评估方法及其关键参数。
- 条件与环境：时间、温度、压力、剂量、浓度、载荷、频率、设备、模型、软件、协议等可复现实验条件。
- 结果与指标：论文报告的核心量化结果、性能指标、误差/不确定性、分类结果、观察结论。
- 机制与表征：图像、谱图、显微/形貌、曲线、统计图、表格、消融/对照中可结构化的观测。
- 计算/模型：算法、模型版本、输入特征、训练/推理设置、边界条件、评价指标等（若该领域涉及）。
...
- from_figure：该字段是否主要来自图/表/图像/曲线/表征说明(true/false)
...
- 优先保留 coverage 高、且属于该领域核心对象/方法/条件/结果的字段。
- 保留一部分高价值但少见的字段(coverage 低但对领域结论、机制解释或复现实验很关键)。
- **保留若干 from_figure=true 的图/表/曲线派生字段**，不要因为来自图表就丢弃。
...
- 给 record_definition：说明「一条记录」代表什么
  （例如：一个研究对象在一组方法/条件下的一次可独立比较的观测或结果）。
...
审查要点：
1. 覆盖度：领域核心维度(研究对象/对象属性/方法/条件/结果/表征或机制/计算模型)是否齐全？缺则补。
2. 冗余度：是否有重复或可合并字段？合并并保留更通用的命名。
3. 可抽取性：每个字段是否能从论文中较稳定地抽到？过于主观/罕见无价值的删除。
4. 类型与单位：type/unit/enum_values 是否合理、规范？
5. 图/表字段：至少保留若干 from_figure=true 字段(例如图像观察、曲线峰值、表格派生指标、关键图注结论)。
6. 字段总数严格落在 {min}~{max}。
